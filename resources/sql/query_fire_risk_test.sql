...
-- radius: 8000
-- power_factor: 0.2
WITH coordinate       AS (SELECT ST_Transform(ST_SetSRID(ST_Point(-118.1445155, 34.1477849), 4326), 2163) AS geom),
     buffer           AS (SELECT ST_Buffer(geom, 8000) AS geom
                            FROM coordinate),
     ...
     pixel_weights    AS (SELECT layer, val, (1/distance^0.2)/sum(1/distance^0.2) OVER (PARTITION BY layer) AS weight
