...
-- radius: 16000
-- power_factor: 0.2
WITH coordinate           AS (SELECT ST_Transform(ST_SetSRID(ST_Point(-119.9771868, 38.939926), 4326), 2163) AS geom),
     ...
     buffer_stations      AS (SELECT wrcc, ST_Distance(c.geom, s.geom) AS distance
                                FROM coordinate AS c
                                INNER JOIN weather.raws_points AS s ON ST_DWithin(c.geom, s.geom, 16000)),
     station_ffwis        AS (SELECT avg_ffwi, stddev_ffwi, (1/distance^0.2)/sum(1/distance^0.2) OVER () AS weight
